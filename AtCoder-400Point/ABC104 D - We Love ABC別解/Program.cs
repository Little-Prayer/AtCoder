...
namespace ABC104_D___We_Love_ABC別解
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var accumA = new long[S.Length + 1];
            var accumC = new long[S.Length + 1];
            var accumQ = new long[S.Length + 1];
            ...
            for (int i = 1; i < S.Length; i++)
            {
                if (S[i - 1] == 'B')
                {
                    result += accumA[i - 1] * (totalC - accumC[i]) * power3Q;
                    result += accumA[i - 1] * (totalQ - accumQ[i]) * power3Qminus1;
                    result += accumQ[i - 1] * (totalC - accumC[i]) * power3Qminus1;
                    result += accumQ[i - 1] * (totalQ - accumQ[i]) * power3Qminus2;
                }
                if (S[i - 1] == '?')
                {
                    result += accumA[i - 1] * (totalC - accumC[i]) * power3Qminus1;
                    result += accumA[i - 1] * (totalQ - accumQ[i]) * power3Qminus2;
                    result += accumQ[i - 1] * (totalC - accumC[i]) * power3Qminus2;
                    result += accumQ[i - 1] * (totalQ - accumQ[i]) * power3Qminus3;
                }
            }
